const p = new URLSearchParams(window.location.search),
  c = p.get("share") || window.location.pathname.split("/").pop() || "",
  a = document.getElementById("modal"),
  s = document.getElementById("modal-content"),
  h = document.querySelector(".close");
async function g() {
  try {
    const e = p.get("share") || window.location.pathname.split("/").pop() || "";
    if (!e) throw new Error("No share hash found");
    try {
      const n = await fetch(`/api/share/${e}/files`);
      if (n.ok) {
    ...
    for (const n of i)
      try {
        const t = `/api/share/${e}/file/${n.name}`;
        (await fetch(t, { method: "HEAD" })).ok && o.push(n);
      } catch {
        console.log(`File ${n.name} not accessible via primary URL`);
        try {
          const t = `/api/public/dl/${e}/${n.name}`;
          (await fetch(t, { method: "HEAD" })).ok && o.push({ ...n, altUrl: t });
        } catch {
          console.log(`File ${n.name} not accessible via alternative URL either`);
        }
      }
    o.length > 0 ? r(o) : r(i);
  } catch (e) {
    (console.error("Error loading media files:", e),
      m(`Failed to load media files: ${e instanceof Error ? e.message : String(e)}`));
  }
}
function r(e) {
  const i = document.getElementById("media-container"),
...
function v(e) {
  ...
  i.className = "media-card";
  const o = e.type === "video",
    n =
      new URLSearchParams(window.location.search).get("share") ||
      window.location.pathname.split("/").pop() ||
      "",
    t = e.inline_url || `/api/public/dl/${n}/${e.name}?inline=true`,
    l = e.download_url || `/api/share/${n}/file/${e.name}?download=1`,
    y = `/api/share/${n}/preview/${e.name}`;
  i.innerHTML = `
    ${
      o
        ...
        : `<img class="media-preview" src="${t}" alt="${e.name}"
  ...
  `;
  const d = i.querySelector(".media-preview");
  return (
    d &&
...
function u(e, i) {
  if (e.type === "video") {
    ...
    `;
    const n = s.querySelector("video"),
      t = s.querySelector(".video-loader");
    n &&
      t &&
      ((n.onloadstart = () => {
    ...
    s.innerHTML = `<img src="${i}" alt="${e.name}" style="max-width: 100%; max-height: 100%;">`;
  a.style.display = "block";
}
function m(e = "Please check your share link and try again.") {
  const i = document.getElementById("loading"),
    o = document.getElementById("error"),
    n = o.querySelector("p");
  ((i.style.display = "none"), (o.style.display = "block"), n && (n.textContent = e));
}
...
document.addEventListener("keydown", (e) => {
  e.key === "Escape" && (a.style.display = "none");
});
c && c !== "media-viewer.html"
  ? g()
  : m("No valid share hash found in URL. Please use a proper share link.");
