(function () {
  ...
  var accountsPasswordRules = null;
  ...
  var DROPPR_ACCESS_TOKEN_KEY = "droppr_access_token";
  var DROPPR_REFRESH_TOKEN_KEY = "droppr_refresh_token";
  var DROPPR_OTP_KEY = "droppr_otp_code";
  var SESSION_WARNING_ID = "droppr-session-warning";
  var SESSION_WARNING_STYLE_ID = "droppr-session-warning-style";
  var SESSION_WARNING_THRESHOLD_MS = 5 * 60 * 1000;
  var dropprAuthPromise = null;
  ...
  function decodeJwtPayload(token) {
    ...
    try {
      ...
      var pad = payload.length % 4 ? "====".slice(payload.length % 4) : "";
      var json = atob(payload + pad);
  ...
  function getDropprTokenState() {
    ...
    var accessPayload = decodeJwtPayload(access);
    var refreshPayload = decodeJwtPayload(refresh);
    return {
      access: access,
      refresh: refresh,
  ...
  function saveDropprTokens(access, refresh) {
    try {
  ...
  function setDropprOtpCode(code) {
    try {
      if (code) {
  ...
  function promptForOtp() {
    ...
    if (code) {
      setDropprOtpCode(code);
      return code;
    }
    return "";
  }

  function ensureSessionWarningStyles() {
  ...
  function ensureSessionWarning() {
    ensureSessionWarningStyles();
    var existing = document.getElementById(SESSION_WARNING_ID);
    if (existing) return existing;
    var el = document.createElement("div");
  ...
  function getSessionExpiryMs() {
    var state = getDropprTokenState();
    if (state.access && state.accessExp) return state.accessExp;
    var fbToken = getAuthToken();
    var payload = decodeJwtPayload(fbToken);
    if (payload && payload.exp) return payload.exp * 1000;
    return 0;
  }

  function updateSessionWarning() {
    var el = document.getElementById(SESSION_WARNING_ID);
    if (!el) return;
    var exp = getSessionExpiryMs();
    if (!exp) {
  ...
  function parseDropprAuthResponse(res) {
  ...
  function loginDropprToken(allowPrompt) {
    var fbToken = getAuthToken();
    if (!fbToken) return Promise.resolve(null);
    var headers = { "X-Auth": fbToken };
    var otp = getDropprOtpCode();
    if (otp) headers["X-Droppr-OTP"] = otp;
    return fetch("/api/droppr/auth/login", { method: "POST", headers: headers })
      .then(parseDropprAuthResponse)
      .then(function (result) {
        if (result.res.status === 401 && result.data && result.data.otp_required && allowPrompt) {
          var code = promptForOtp();
          if (!code) return null;
          return loginDropprToken(false);
        }
        if (!result.res.ok) return null;
        if (result.data && result.data.access_token) {
          saveDropprTokens(result.data.access_token, result.data.refresh_token);
  ...
  function refreshDropprToken(refreshToken, allowPrompt) {
    ...
    return fetch("/api/droppr/auth/refresh", { method: "POST", headers: headers })
      ...
      .then(function (result) {
        ...
        if (!result.res.ok) {
          if (result.res.status === 401) clearDropprTokens();
          return null;
        }
        if (result.data && result.data.access_token) {
          saveDropprTokens(result.data.access_token, result.data.refresh_token);
  ...
  function ensureDropprAccessToken(force) {
    if (dropprAuthPromise) return dropprAuthPromise;
    var state = getDropprTokenState();
    var now = Date.now();
    ...
    dropprAuthPromise = (function () {
      if (!force && state.refresh && state.refreshExp > now + 60000) {
  ...
  function dropprFetch(url, options) {
    var opts = options || {};
    opts.headers = opts.headers || {};
    return ensureDropprAccessToken(false).then(function (accessToken) {
      if (accessToken) {
        opts.headers["Authorization"] = "Bearer " + accessToken;
      } else {
        var fbToken = getAuthToken();
        if (fbToken) opts.headers["X-Auth"] = fbToken;
      }
  ...
  function checkAccountsAdmin() {
    ...
    accountsAdminCheck = dropprFetch("/api/droppr/users", {
      headers: {},
    })
      .then(function (res) {
        return res.text().then(function (text) {
          ...
          if (data && data.password_rules && typeof data.password_rules === "object") {
            accountsPasswordRules = data.password_rules;
          }
  ...
  function showAccountsModal() {
    ...
    var meter = panel.querySelector("#droppr-account-meter");
    var meterLabel = panel.querySelector("#droppr-account-meter-label");
    var meterBar = meter ? meter.querySelector(".bar span") : null;
    ...
    function getPasswordRules() {
      var rules = accountsPasswordRules || {};
      return {
        min_length: Math.max(accountsPasswordMinLen, parseInt(rules.min_length, 10) || 0),
        require_upper: rules.require_upper !== false,
        require_lower: rules.require_lower !== false,
        require_digit: rules.require_digit !== false,
        require_symbol: rules.require_symbol !== false,
        pwned_check: !!rules.pwned_check,
      };
    }

    function passwordRulesError(value) {
      var rules = getPasswordRules();
      var password = String(value || "");
      if (password.length < rules.min_length) {
        return "Password must be at least " + rules.min_length + " characters.";
      }
      if (rules.require_upper && !/[A-Z]/.test(password)) {
        return "Password must include an uppercase letter.";
      }
      if (rules.require_lower && !/[a-z]/.test(password)) {
        return "Password must include a lowercase letter.";
      }
      if (rules.require_digit && !/[0-9]/.test(password)) {
        return "Password must include a number.";
      }
      if (rules.require_symbol && !/[^A-Za-z0-9]/.test(password)) {
    ...
    function updatePasswordMeter() {
      ...
      var checks = [];
      checks.push({ ok: password.length >= rules.min_length, label: "length" });
      if (rules.require_upper) checks.push({ ok: /[A-Z]/.test(password), label: "upper" });
      if (rules.require_lower) checks.push({ ok: /[a-z]/.test(password), label: "lower" });
      ...
      var suffix = rules.pwned_check ? " • breach check enabled" : "";
      meterLabel.textContent = "Strength: " + label + " (" + passed + "/" + total + ")" + suffix;
    }
    ...
    function submit() {
      ...
      var passwordError = passwordRulesError(password);
      if (passwordError) {
  ...
  function showRequestModal() {
    ...
    function submit() {
      ...
      dropprFetch("/api/droppr/requests", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  function buildDropprPreviewUrl(path, width) {
    var url = "/api/droppr/preview?path=" + encodeURIComponent(String(path || ""));
    if (width) url += "&w=" + encodeURIComponent(String(width));
    url += "&format=auto";
    return url;
  }
  ...
  function applyVideoThumbnail(rowEl, path, nameEl) {
    ...
    var url = buildDropprPreviewUrl(path, DROPPR_THUMB_WIDTH);
    var url2x = buildDropprPreviewUrl(path, DROPPR_THUMB_WIDTH * 2);
  ...
  function fetchVideoMeta(path) {
    ...
    return dropprFetch("/api/droppr/video-meta?path=" + encodeURIComponent(path), opts)
      .then(function (res) {
  ...
  function updateShareExpire(shareHash, hours, sharePath) {
    return dropprFetch("/api/droppr/shares/" + encodeURIComponent(shareHash) + "/expire", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  function boot() {
    ...
    ensureSessionWarning();
    updateSessionWarning();
    if (!sessionWarningTimer) {
      sessionWarningTimer = setInterval(updateSessionWarning, 60000);
    }
