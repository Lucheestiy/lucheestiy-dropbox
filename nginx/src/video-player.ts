...
(function (): void {
  ...
  let qualityMode: QualityMode = (params.get('quality') || 'auto').trim().toLowerCase() as QualityMode;
  ...
  function updateStatusUI(): void {
    ...
    if (timeText) timeText.textContent = info.dur ? `${formatTime(info.ct)} / ${formatTime(info.dur)}` : formatTime(info.ct);

    const modeLabel = QUALITY_LABEL[qualityMode] || 'Auto';
    const srcLabel = activeSource ? sourceLabel(activeSource) : '';
    let text = srcLabel ? `${modeLabel} • ${srcLabel} • ${stateLabel}` : `${modeLabel} • ${stateLabel}`;
    ...
    if (supportsHlsJs() && window.Hls) {
      ...
      hlsInstance.on(window.Hls.Events.ERROR, (_: string, data?: HlsErrorData) => {
        if (data && data.fatal) {
          ...
          finishLoad();
          if (qualityMode === 'auto') {
            setSource(pickPlayableSource('fast', { avoid: 'hls' }), { time: targetTime ?? undefined, shouldPlay });
          }
        }
      });
    } else {
  ...
  video.addEventListener('error', () => {
    ...
    showError('Could not load video', 'Try refreshing the page. If it keeps failing, go back to the gallery.');
  });
