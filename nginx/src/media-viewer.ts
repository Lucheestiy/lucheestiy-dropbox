...
async function loadMediaFiles(): Promise<void> {
  try {
    ...
    const currentShareHash =
      urlParams.get("share") || window.location.pathname.split("/").pop() || "";
    if (!currentShareHash) {
    ...
    showError(
      `Failed to load media files: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
function createMediaCard(file: MediaFile): HTMLElement {
  ...
  const downloadUrl =
    file.download_url || `/api/share/${currentShareHash}/file/${file.name}?download=1`;
