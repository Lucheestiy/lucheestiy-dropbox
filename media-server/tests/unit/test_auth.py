...
def test_password_rules_error(app_module):
    assert app_module._password_rules_error(None) == "Missing password"
    assert "at least" in app_module._password_rules_error("short")
    assert "uppercase" in app_module._password_rules_error("lowercase1!")
    assert "number" in app_module._password_rules_error("NoNumber!")
    assert "symbol" in app_module._password_rules_error("NoSymbol1")
    assert app_module._password_rules_error("GoodPass1!") is None


def test_jwt_encode_decode(app_module):
    secret = "secret"
    payload = {"sub": "user", "exp": int(time.time()) + 60}
    token = app_module._encode_jwt(payload, secret)
    decoded = app_module._decode_jwt(token, secret)
    assert decoded["sub"] == "user"

    expired = app_module._encode_jwt({"sub": "user", "exp": int(time.time()) - 5}, secret)
    assert app_module._decode_jwt(expired, secret) is None

    assert app_module._peek_jwt_payload(token)["sub"] == "user"
    assert app_module._peek_jwt_payload("bad.token") is None
...
def test_admin_ip_allowlist(app_module):
    app_module.ADMIN_IP_ALLOWLIST[:] = [app_module.ipaddress.ip_network("10.0.0.0/8")]
